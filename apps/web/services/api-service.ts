...
export class APIError extends Error {
  ...
  get message(): string {
    switch (this.status) {
      ...
      default:
        return (
          this.statusText || 'An error occurred while processing your request.'
        );
    }
  }
...
export async function searchPatients(query: string): Promise<Patient[]> {
  return apiCall<Patient[]>(
    `/patients/search?search=${encodeURIComponent(query)}`
  );
}
...
export async function apiCall<T>(
  ...
  try {
    const apiBaseUrl =
      process.env.NEXT_PUBLIC_API_SERVER || 'http://localhost:4000';
